...
const startServer = async () => {
  try {
    ...
    if (!isClickHouseHealthy) {
      ...
      // Initialize application registry tables
      const { applicationRegistryService } = await import('./services/applicationRegistryService');
      logger.info('Initializing application registry...');
      await applicationRegistryService.initialize();
      logger.info('Application registry initialized successfully');
    }
